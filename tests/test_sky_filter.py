...
def test_sky_filter(test_image_path, cache_dir):
    ...
    print(
        f"Raw output - min: {sky_pred.min():.4f}, max: {sky_pred.max():.4f}, mean: {sky_pred.mean():.4f}"
    )
    ...
    print(
        f"Normalized output - min: {sky_pred.min()}, max: {sky_pred.max()}, mean: {sky_pred.mean():.2f}"
    )
    ...
    print(
        f"Sky points (< {threshold}): {sky_points} out of {total_points} ({sky_percentage:.2f}%)"
    )
    print(
        f"Non-sky points (>= {threshold}): {total_points - sky_points} ({100-sky_percentage:.2f}%)"
    )
